...
it('starts a workflow', function () {
    ...
    $instance = $service->startWorkflow($this->contract, $this->template);
...
it('prevents duplicate active workflows', function () {
    $service = new WorkflowService();
    $service->startWorkflow($this->contract, $this->template);
    expect(fn () => $service->startWorkflow($this->contract, $this->template))
...
it('advances workflow on approve', function () {
    $service = new WorkflowService();
    $instance = $service->startWorkflow($this->contract, $this->template);
    $updated = $service->performAction($instance, 'approve');
    expect($updated->current_stage)->toBe('Sign');
});

it('completes workflow at last stage', function () {
    $service = new WorkflowService();
    $instance = $service->startWorkflow($this->contract, $this->template);
    $instance = $service->performAction($instance, 'approve');
    $instance = $service->performAction($instance, 'approve');
    expect($instance->state)->toBe('completed');
    expect($this->contract->fresh()->workflow_state)->toBe('executed');
});
...
it('rejects back to previous stage', function () {
    ...
    $instance = $service->startWorkflow($this->contract, $this->template);
    $instance = $service->performAction($instance, 'approve');
    $instance = $service->performAction($instance, 'reject');
    expect($instance->current_stage)->toBe('Review');
});
