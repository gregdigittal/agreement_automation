...
async def process_due_reminders(supabase: Client) -> int:
    ...
    for reminder in result.data or []:
        try:
            supabase.table("notifications").insert(
                {
                    ...
                    "recipient_user_id": reminder.get("recipient_user_id"),
                    "channel": reminder["channel"],
                    "subject": "CCRS Reminder: " + reminder["reminder_type"] + " for contract",
                    "body": (
                        "Reminder for contract "
                        + str(reminder.get("contracts", {}).get("title", reminder["contract_id"]))
                        + ". Type: "
                        + reminder["reminder_type"]
                        + ". Lead time: "
                        + str(reminder["lead_days"])
                        + " days."
                    ),
                    "related_resource_type": "contract",
                    "related_resource_id": reminder["contract_id"],
            ...
            supabase.table("reminders").update({"last_sent_at": now}).eq(
                "id", reminder["id"]
            ).execute()
