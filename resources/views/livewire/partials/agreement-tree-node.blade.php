...
<div role="treeitem" aria-expanded="{{ $isExpanded ? 'true' : 'false' }}">
    {{-- Node row --}}
    <button
        wire:click="toggleNode('{{ $nodeKey }}');{{ !$isExpanded && !$hasChildren ? "loadContracts('{$node['type']}', '{$node['id']}')" : '' }}"
    ...
    @if ($isExpanded)
        ...
        {{-- Contract rows (only show at leaf nodes — skip when node has children to avoid duplicates) --}}
        @if (!$hasChildren)
            @php
                $contractData = $loadedContracts[$nodeKey] ?? [];
                $contracts = $contractData['contracts'] ?? [];
                $totalContracts = $contractData['total'] ?? 0;
                $showingContracts = $contractData['showing'] ?? 0;
            @endphp
            ...
            @if (count($contracts) > 0)
                <div class="border-b border-gray-100 dark:border-gray-700">
                    ...
                    @if ($totalContracts > $showingContracts)
                        <div class="px-3 py-2 {{ $level === 0 ? 'pl-12' : ($level === 1 ? 'pl-[4.5rem]' : 'pl-24') }} text-xs text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-750 border-t border-gray-100 dark:border-gray-700">
                            Showing {{ $showingContracts }} of {{ $totalContracts }} contracts.
                        </div>
                    @endif
                </div>
            @else
